...
{
    struct dynamic_string *dynarray = (struct dynamic_string *) *darr;
...
{
    struct dynamic_string *dynarray = malloc(sizeof(struct dynamic_string) + sizeof(char) * (length - 1));
    if(dynarray == NULL)
        return NULL;
...
bool String_insert(String **darr, char val, size_t index)
{
    ...
    {
        struct dynamic_string *realloced_dynarray = (struct dynamic_string *) realloc(dynarray, sizeof(struct dynamic_string) + dynarray->size * STRING_REALLOC_FACTOR * sizeof(char));
...
bool String_resize(String **darr, size_t size)
{
    struct dynamic_string *dynarray = (struct dynamic_string *) *darr;
    assert(dynarray != NULL); 
    struct dynamic_string *realloced_dynarray = (struct dynamic_string *) realloc(dynarray, sizeof(struct dynamic_string) + size * sizeof(char));
    if(realloced_dynarray == NULL)
        return false;
...
bool String_adjust_length(String **darr, size_t length)
{
    ...
    if(length > dynarray->size)
    {
        ...
        dynarray = (struct dynamic_string *) *darr;
    }
    dynarray->length = length;
    return true;
}
...
{
    struct dynamic_string *dynarray_src = (struct dynamic_string *) src;
